...
public class AddManfaat extends AppCompatActivity {
        ...
        if (id == R.id.action_save) {
            if (data == null) {
                ...
                if (data.getImage() == txtInsert.getText().toString()) {
                    updateWithField(data.getIdManfaat());
                    Toast.makeText(this, data.getIdManfaat(), Toast.LENGTH_SHORT).show();
                } else {
                    updateWithImage(data.getIdManfaat());
                    Toast.makeText(this, data.getIdManfaat(), Toast.LENGTH_SHORT).show();
                }
            }
    ...
    private void simpan() {
        ...
        Call<ResponseInsert> call = service.insertData(1, partImage, edtJudul.getText().toString(),
                editor.getCachedHtml(), getDateTime());
        call.enqueue(new Callback<ResponseInsert>() {
            @Override
            public void onResponse(Call<ResponseInsert> call, Response<ResponseInsert> response) {
                if (response.body().getCode() == 200) {
                    Toast.makeText(AddManfaat.this, response.body().getMessage(), Toast.LENGTH_SHORT).show();
                } else {
                    Toast.makeText(AddManfaat.this, response.body().getMessage(), Toast.LENGTH_SHORT).show();
                }
            }
            ...
            public void onFailure(Call<ResponseInsert> call, Throwable t) {
                Toast.makeText(AddManfaat.this, "" + t.getMessage(), Toast.LENGTH_SHORT).show();
            }
        });
    }
    ...
    private void updateWithImage(String id) {
        ...
        //eksekusi
        ApiService service = RetroClient.getApiService();
        Call<ResponseInsert> call = service.updateManfaat1(Integer.parseInt(id), partImage, edtJudul.getText().toString(),
    ...
    private void updateWithField(String id) {
        ApiService service = RetroClient.getApiService();
        Call<ResponseInsert> call = service.updateManfaat2(Integer.parseInt(id), edtJudul.getText().toString(), removeString(editor.getCachedHtml()));
